...
pub fn send_infomsg(
    ...
    if tls_send {
        tls_send_to(storage, socket_id, BufferType::MBuffer(buf))
    } else {
        send_to(storage, socket_id, BufferType::MBuffer(buf))
    }
}
...
pub fn send_move_ok(storage: &Storage, socket_id: usize, move_ok: bool) -> Result<()> {
    ...
    send_to_front(storage, socket_id, BufferType::MBuffer(buf))
}
...
pub fn send_codes(
    ...
    tls_send_to(storage, id, BufferType::MBuffer(buf))
}
...
pub fn send_ping(world: &mut World, storage: &Storage, entity: &Entity) -> Result<()> {
    ...
    send_to(storage, id, BufferType::MBuffer(buf))
}
...
pub fn send_message(
    ...
    match chan {
        ...
        MessageChannel::Private => {
            ...
            if let Some(i) = id {
                send_to(storage, i, BufferType::Buffer(buf.clone()))?;
            }
            send_to(
                storage,
                world.get::<&Socket>(entity.0)?.id,
                BufferType::Buffer(buf),
            )?;
        }
...
pub fn send_updatetradeitem(
    ...
    send_to(
        storage,
        world.get::<&Socket>(send_entity.0)?.id,
        BufferType::MBuffer(buf),
    )
}
...
pub fn send_traderequest(
    ...
    send_to(
        storage,
        world.get::<&Socket>(target_entity.0)?.id,
        BufferType::MBuffer(buf),
    )
}
