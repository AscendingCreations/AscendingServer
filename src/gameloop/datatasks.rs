...
pub fn init_data_lists(world: &Storage, user: &mut Player, oldmap: MapPosition) {
    ...
    while let Some(i) = user.datatasks.pop() {
        world.datatasks.borrow_mut().remove(i);
    }
    ...
    user.datatasks.push(
        world
            .datatasks
            .borrow_mut()
            .insert(DataTasks::Player(task_player)),
    );
    user.datatasks.push(
        world
            .datatasks
            .borrow_mut()
            .insert(DataTasks::Npc(task_npc)),
    );
    user.datatasks.push(
        world
            .datatasks
            .borrow_mut()
            .insert(DataTasks::Items(task_item)),
    );
}
